...
class JagUtils:
	...
	def cryToRgba32(data, flags = 0):
		return rapi.callExtensionMethod("jaguar_cry_to_rgba32", data, flags)
	...
	def rgba32ToCry(data, flags = 0):
		return rapi.callExtensionMethod("jaguar_rgba32_to_cry", data, flags)

	#returns the closest (without going under) supported texture width
	def closestTextureWidth(width):
		return rapi.callExtensionMethod("jaguar_closest_width", width)
	def decodeTextureWidth(flags):
		return rapi.callExtensionMethod("jaguar_decode_width", flags)
	...
	def lzjagDecomp(data, decompSize):
		return rapi.callExtensionMethod("jaguar_lzjag_decomp", data, decompSize)
	def lzjagDecompSize(data):
		return rapi.callExtensionMethod("jaguar_lzjag_decomp_size", data)
